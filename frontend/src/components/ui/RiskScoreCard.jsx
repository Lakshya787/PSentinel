...
/**
 * RiskScoreCard — circular gauge + level label with animations.
...
export default function RiskScoreCard({ score, level, size = 'md' }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div
        className={`relative rounded-full transition-shadow ${isCritical ? 'animate-ring-pulse' : ''}`}
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`font-black leading-none tabular-nums ${fSize}`} style={{ color: hex }}>
            {score}
          </span>
          <span className={`text-slate-400 font-medium ${lSize}`}>/100</span>
        </div>
      </div>
      <span className={`text-xs font-black px-3 py-1 rounded-full border uppercase tracking-widest ${cls}`}>
        {level}
      </span>
    </div>
  )
