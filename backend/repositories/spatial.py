"""
repositories/spatial.py — All five required PostGIS queries (ref.md §11).
...
def neighbouring_reports(
    ...
    EXPLAIN ANALYZE on this query will show: Index Scan using ix_reports_geom on reports
    (GIST index accelerates the ST_DWithin bounding-box filter).
    """
    sql = text("""
        SELECT
            r.id,
...
def compute_spatial_risk_factor(neighbours: list[dict]) -> float:
    ...
    # Base: 40 + 15 per neighbour, capped at 100
    base = min(100.0, 40.0 + len(neighbours) * 15.0)

    # Proximity boost: each neighbour within 5 km adds up to 5 points
...
def create_containment_rings(
    ...
    Also returns villages inside each ring via ST_DWithin, with fmd_vax_coverage
    for the ring-vaccination priority list and alert recipients (ref.md §17).
    """
    results = []

    for ring_km, radius_m in [(3, RING_3KM_METRES), (10, RING_10KM_METRES)]:

        # Insert ring polygon (idempotent via UNIQUE(report_id, ring_km))
        zone_sql = text("""
        ...
        # Villages inside ring (ST_DWithin on villages.geom)
        vil_sql = text("""
            SELECT
                v.name,
                v.taluk,
...
def postgis_cluster_dbscan(db: Session) -> dict:
    ...
    This replaces the Python-level haversine ST-DBSCAN previously in ml_engine.py
    for the /clusters endpoint (ref.md §11).
    """
    # Main clustering query
    cluster_sql = text("""
        WITH clustered AS (
            SELECT
...
def explain_dwithin(db: Session, lat: float, lon: float, syndrome: str, radius_m: float = 15_000) -> list[str]:
